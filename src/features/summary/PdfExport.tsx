...
): { type: string; width: number; height: number; position: number; offsetY: number }[] {
...
async function renderPdf(data: PdfData, doc: InstanceType<typeof import('jspdf').jsPDF>) {
  ...
  doc.roundedRect(margin, yPos, contentW, 14, 2, 2, 'FD');

  const boxY = yPos + 6;
  doc.setFontSize(10);
  doc.setTextColor(...C.dark);
  doc.text(`Gesamtmaße: ${data.totalDimensions}`, margin + 6, boxY);
  doc.text('Innenhöhe: 2,10 m | Außenhöhe: 2,50 m', margin + contentW * 0.35, boxY);
  doc.text(`Module: ${data.modules.length}`, margin + contentW * 0.75, boxY);
  ...
  for (let idx = 0; idx < data.modules.length; idx++) {
    ...
    const mDef = MODULE_DEFINITIONS[m.type];
    const mW = (m.width * GRID_CELL_SIZE).toFixed(1);
    const mD = (m.height * GRID_CELL_SIZE).toFixed(1);
    const mPrice = calculateModulePrice(m);

    yPos = ensureSpace(yPos, 6);
    doc.setFontSize(9);
    doc.setTextColor(...C.dark);
    doc.text(`${mDef?.name ?? m.type} #${idx + 1}  (${mW} × ${mD} m)`, margin + 4, yPos);
    doc.text(formatPrice(mPrice), pageW - margin, yPos, { align: 'right' });
    yPos += 5;
  }
  ...
  doc.setFontSize(11);
  doc.setTextColor(...C.brand);
  doc.text('Gesamtpreis (Richtwert)', margin + 4, yPos);
  doc.setFontSize(12);
  doc.text(formatPrice(totalPrice), pageW - margin, yPos, { align: 'right' });
  yPos += 8;
