...
function InteractiveFloorPlan({
  ...
  wallConfig: WallConfig;
  sharedWalls: Set<WallSide>;
  widthM: number;
  depthM: number;
  selectedSide: WallSide;
  onSelectSide: (side: WallSide) => void;
}) {
  const svgW = 160;
  const svgH = 130;
  const pad = 24;
  const scale = Math.min((svgW - pad * 2) / widthM, (svgH - pad * 2) / depthM);
  const rw = widthM * scale;
  const rh = depthM * scale;
  const rx = (svgW - rw) / 2;
  const ry = (svgH - rh) / 2;

  const wallColor = (side: WallSide): string => {
    if (sharedWalls.has(side)) {
      const intState = getInteriorState(wallConfig, side);
      if (intState === 'open') return '#d1d5db';
      if (intState === 'door' || intState === 'terrace-door') return '#059669';
      return '#8B6914';
    }
    const state = getWallState(wallConfig[side]);
    if (state === 'door') return '#d97706';
    if (state === 'terrace-door') return '#059669';
    if (state === 'window' || state === 'window-normal') return '#2563eb';
    return '#6B5420';
  };

  const strokeDash = (side: WallSide): string | undefined => {
    if (sharedWalls.has(side)) {
      const intState = getInteriorState(wallConfig, side);
      if (intState !== 'open') return '4 2';
    }
    return undefined;
  };

  // Wall icon indicator (small symbol near wall center)
  const getWallIcon = (side: WallSide): string => {
    if (sharedWalls.has(side)) {
      const intState = getInteriorState(wallConfig, side);
      if (intState === 'open') return '';
      if (intState === 'door') return 'D';
      if (intState === 'terrace-door') return 'T';
      return '';
    }
    const state = getWallState(wallConfig[side]);
    if (state === 'window') return 'F';
    if (state === 'window-normal') return 'F';
    if (state === 'door') return 'D';
    if (state === 'terrace-door') return 'T';
    return '';
  };

  // Clickable areas for each wall
  const clickZoneThickness = 16;
  ...
  const zones: WallZone[] = [
    {
      side: 'front',
      x: rx, y: ry + rh - clickZoneThickness / 2,
      width: rw, height: clickZoneThickness,
      lineX1: rx, lineY1: ry + rh, lineX2: rx + rw, lineY2: ry + rh,
      iconX: rx + rw / 2, iconY: ry + rh - 6,
      labelX: svgW / 2, labelY: ry + rh + 16, label: 'V',
    },
    {
      side: 'back',
      x: rx, y: ry - clickZoneThickness / 2,
      width: rw, height: clickZoneThickness,
      lineX1: rx, lineY1: ry, lineX2: rx + rw, lineY2: ry,
      iconX: rx + rw / 2, iconY: ry + 10,
      labelX: svgW / 2, labelY: ry - 8, label: 'H',
    },
    {
      side: 'left',
      x: rx - clickZoneThickness / 2, y: ry,
      width: clickZoneThickness, height: rh,
      lineX1: rx, lineY1: ry, lineX2: rx, lineY2: ry + rh,
      iconX: rx + 8, iconY: ry + rh / 2 + 3,
      labelX: rx - 12, labelY: svgH / 2 + 3, label: 'L',
    },
    {
      side: 'right',
      x: rx + rw - clickZoneThickness / 2, y: ry,
      width: clickZoneThickness, height: rh,
      lineX1: rx + rw, lineY1: ry, lineX2: rx + rw, lineY2: ry + rh,
      iconX: rx + rw - 8, iconY: ry + rh / 2 + 3,
      labelX: rx + rw + 12, labelY: svgH / 2 + 3, label: 'R',
    },
  ];
  ...
  return (
    <svg width={svgW} height={svgH} className="shrink-0">
      {/* Fill */}
      <rect x={rx} y={ry} width={rw} height={rh} fill="#f5f0e8" rx={2} />

      {/* Wall lines + clickable zones */}
      {zones.map((z) => {
        const isSelected = selectedSide === z.side;
        const color = wallColor(z.side);
        const dash = strokeDash(z.side);
        const icon = getWallIcon(z.side);

        return (
          <g key={z.side}>
            {/* Clickable zone (invisible rect) */}
            <rect
              x={z.x}
              y={z.y}
              width={z.width}
              ...
              onClick={() => onSelectSide(z.side)}
            />

            {/* Selected highlight glow */}
            {isSelected && (
              <line
                x1={z.lineX1} y1={z.lineY1}
                x2={z.lineX2} y2={z.lineY2}
                stroke="#3b82f6"
                strokeWidth={10}
                opacity={0.15}
                strokeLinecap="round"
